...
class Gather(General):
    ...
    methods = {"getServerTime": "/api/v1/time", "getExchangeInfo": "/api/v1/exchangeInfo", "getOrderBook": "/api/v1/depth",
               "getTradeList": "/api/v1/trades", "getOldTrades": "/api/v1/historicalTrades", "getAggregateList": "/api/v1/historicalTrades",
               "getCandlestickInfo": "/api/v1/klines", "getPriceTicker": "/api/v1/ticker/24hr", "getLastPriceTicker": "/api/v3/ticker/price",
               "getBestPrice": "/api/v3/ticker/bookTicker"}
    # Test connectivity to the Rest API and get the current server time. Return: serverTime
    def getServerTime(self):
        r = requests.get(self.api + self.methods["getServerTime"], verify=False)
        return r.json()

    # Current exchange trading rules and symbol information. Return: see documentation.
    def getExchangeInfo(self):
        r = requests.get(self.api + self.methods["getExchangeInfo"], verify=False)
        return r.json()
    ...
    def getOrderBook(self, v_symbol, v_limit=100):
        r = requests.get(self.api + self.methods["getOrderBook"], params={"symbol": v_symbol, "limit": v_limit}, verify=False)
        return r.json()

    # Get recent trades (up to last 500). Params: limit (Default 500; max 500), symbol(pair)
    def getTradeList(self, v_symbol, v_limit=500):
        r = requests.get(self.api + self.methods["getTradeList"], params={"symbol": v_symbol, "limit": v_limit}, verify=False)
        return r.json()
    ...
    def getOldTrades(self, v_symbol, v_limit=500, v_fromId=None):
        r = requests.get(self.api + self.methods["getOldTrades"], params={"symbol": v_symbol, "limit": v_limit, "fromId": v_fromId}, verify=False)
        return r.json()
    ...
    def getAggregateList(self, v_symbol, v_fromId=None, v_startTime=None, v_endTime=None, v_limit=500):
        r = requests.get(self.api + self.methods["getAggregateList"], params={"symbol": v_symbol, "fromId": v_fromId, "startTime": v_startTime,
    ...
    def getCandlestickInfo(self, v_symbol, v_interval, v_startTime=None, v_endTime=None, v_limit=500):
        r = requests.get(self.api + self.methods["getCandlestickInfo"], params={"symbol": v_symbol, "interval": v_interval, "startTime": v_startTime,
                                                                                "endTime": v_endTime, "limit": v_limit}, verify=False)
        return r.json()
    ...
    def getPriceTicker(self, v_symbol=None):
        r = requests.get(self.api + self.methods["getPriceTicker"], params={"symbol": v_symbol}, verify=False)
        return r.json()

    # Latest price for a symbol or symbols. Params: optional (symbol)
    def getLastPriceTicker(self, v_symbol=None):
        r = requests.get(self.api + self.methods["getLastPriceTicker"], params={"symbol": v_symbol}, verify=False)
        return r.json()

    # Best price/qty on the order book for a symbol or symbols. Params: optional (symbol)
    def getBestPrice(self, v_symbol=None):
        r = requests.get(self.api + self.methods["getBestPrice"], params={"symbol": v_symbol}, verify=False)
        return r.json()
